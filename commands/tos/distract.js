module.exports = {
    ...
    execute(message) {
        ...
        if (!message.mentions.members.size) return message.channel.send("I'm sorry, but you can't distract yourself.");
        if (message.mentions.members.size > 1) return message.reply("You're attractive, but not that attractive.");
        ...
        message.mentions.members.map(user => {
            ...
            message.author.send(`You have decided to distract ${user.username} for tonight.`);
        });
    }
}