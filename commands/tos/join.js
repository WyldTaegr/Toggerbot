module.exports = {
    ...
    execute(message) {
        ...
        if (!game.starting) return message.channel.send(`Oops, you're too late, ${message.author}!`);
        if (game.players.includes(message.member)) return message.reply("You're already in the game!");
        game.players.push(message.member);
        message.channel.send(`${message.member.nickname ? message.member.nickname : message.author.username} has joined the game`)
