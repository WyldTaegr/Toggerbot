module.exports = {
    ...
    execute(message) {
        ...
        if (message.channel.type == 'text') return message.reply("Wow you're an Investigator?");
        ...
        if (!message.mentions.users.size) return message.channel.send("You must investigate another person!");
        if (message.mentions.users.size > 1) return message.channel.send("You can only investigate one person at a time.");
        ...
        message.mentions.users.map(user => {
            ...
            message.channel.send(`You have decided to investigate ${user.username} for tonight.`);
        });
    }
}