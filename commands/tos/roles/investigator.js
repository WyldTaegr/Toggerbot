...
const action = (action) => {
    ...
    const objectC = game.assignments.get(action.agent);
    const objectT = game.assignments.get(action.receiver);
    objectT.visited.push(objectC)
    const role = objectT.name;
    if (role === 'vigilante' || role === 'veteran' || role === 'mafioso') {
        action.agent.user.send("Your target could be a `Vigilante`, `Veteran`, or `Mafioso`.");
    } else if (role === 'medium' || role === 'janitor' || role === 'retributionist') {
        action.agent.user.send('Your target could be a `Medium`, `Janitor`, or `Retributionist`.');
    } else if (role === 'survivor' || role === 'vampire hunter' || role === 'amnesiac') {
        action.agent.user.send('Your target could be a `Survivor`, `Vampire Hunter`, or `Amnesiac`.');
    } else if (role === 'spy' || role === 'blackmailer' || role === 'jailor') {
        action.agent.user.send('Your target could be a `Spy`, `Blackmailer`, or `Jailor`.');
    } else if (role === 'sheriff' || role === 'executioner' || role === 'werewolf') {
        action.agent.user.send('Your target could be a `Sheriff`, `Executioner`, or `Werewolf`.');
    } else if (role === 'framer' || role === 'vampire' || role === 'jester') {
        action.agent.user.send('Your target could be a `Framer`, `Vampire`, or `Jester`.');
    } else if (role === 'lookout' || role === 'forger' || role === 'witch') {
        action.agent.user.send('Your target could be a `Lookout`, `Forger`, or `Witch`.');
    } else if (role === 'escort' || role === 'transporter' || role === 'consort') {
        action.agent.user.send('Your target could be an `Escort`, `Transporter`, or `Consort`.');
    } else if (role === 'doctor' || role === 'disguiser' || role === 'serial killer') {
        action.agent.user.send('Your target could be a `Doctor`, `Disguiser`, or `Serial Killer`.');
    } else if (role === 'investigator' || role === 'consigliere' || role === 'mayor') {
        action.agent.user.send('Your target could be an `Investigator`, `Consigliere`, or `Mayor`.');
    } else if (role === 'bodyguard' || role === 'godfather' || role === 'arsonist') {
        action.agent.user.send('Your target could be a `Bodyguard`, `Godfather`, or `Arsonist`.');
